		...
		<div class="container">
				...
				<tbody>
					<c:forEach items="${list}" var="dept">
						<tr>
							...
							<td>${dept.deptLoc}</td>
							<td>
								<a href="<%=request.getContextPath() %>/dept/get?deptId=${dept.deptId}" class="btn btn-primary btn-sm">修改</a>
								<a href="<%=request.getContextPath() %>/dept/delete?deptId=${dept.deptId}" class="btn btn-danger btn-sm">删除</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
			...
			<div class="row" style="margin-top: 20px;">
				<div class="col-sm-offset-8 col-sm-4">
					<a href="<%=request.getContextPath() %>/AddDept.html" class="btn btn-success btn-block">增加新部门</a>
				</div>
			</div>
